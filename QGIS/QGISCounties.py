...
for feat in layer.getFeatures():
    try:
        ...
        if len(os.listdir(county_dir)) < 3:
            ...
            filelist = [ f for f in os.listdir(county_dir)]
            for f in filelist:
                os.remove(os.path.join(county_dir, f))

            file_loc = 'D:/Research Files/Building Coords/Microsoft/%s/%s_building_coords.txt' % (name, name)
            uri = 'file:///%s?type=csv&useHeader=No&xField=field_2&yField=field_3&crs=EPSG:4326' % (file_loc)
            ...
            layer.selectByExpression('"COUNTYFP"='+ "'" + str(county) + "'" + ' AND ' + '"STATEFP"=' + "'" + str(statefp) + "'")
            processing.run('native:clip', {'INPUT': buildings, 'OVERLAY': clippath, 'OUTPUT': outpath})

            new_name = county_dir + '/' + str(county) + '_building_coords.txt'
            os.rename(outpath, new_name)
            transform_coords(new_name)
