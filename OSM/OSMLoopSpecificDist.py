...
# Specify a specific cutoff distance to use
target_dist = 125
...
california = california[california['Area'] != 'Stockton'].reset_index(drop=True)
...
def prompt_user():
    ...
    for area_name in areas:
        print('Now working on ' + area_name)
        ...
        if area_name == 'bay':
            area_name = 'Bay Area (needs to be further broken apart)'
        ...
        # Find the OVITO coords with the target dist
        print('\nSorting the data...')
        best_range = round(target_dist/scale, 3)
        print('Finished sorting!')

        # Store the data
        print('\nStoring data...')
        store_data(best_range, reformatted_coords, area_name)
        print('Finished!')
...
def store_data(best_range, coords, area_name):
    directory = coords.rsplit('/', 1)[0]
    directory = directory + '/'
    ...
    export_frame[area_name] = new_col
...
export_path = 'C:/Users/novar/Downloads/Research Stuff/Jupyter Notebooks/Data Exports/OSM_Export_cali_' + str(target_dist) + '.csv'
export_frame.to_csv(path_or_buf=export_path, index=False)
